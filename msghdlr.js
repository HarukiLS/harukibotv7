          	...
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return client.sendMessage(from,`Seu limite de solicitação acabou\n\n_Nota : limite pode ser obtido por meio de ${prefix}buylimit e subindo de nível_`, text,{ quoted: mek})
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					if (isGroupAdmins) return reply('não pode limk, mas cê é adm então pode')
					client.updatePresence(from, Presence.composing)
					if (messagesC.includes("#izinadmin")) return reply("#permissão de administrador recebida<")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`Não pode link de grupo, seu merda ${sender.split("@")[0]} você vai ser banido, seu corno`)
						setTimeout( () => {
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                if(isNaN(umurUser)) return await reply('A idade deve ser um número, caralho!!!')
                if (namaUser.length >= 10) return reply(`nome maior que meu pau vai se fuder`)
                if (umurUser > 25) return reply(`sua idade é muito alta, no máximo 25 anos`)
                if (umurUser < 12) return reply(`sua idade é muito baixa, no mínimo 12 anos`)
                try {
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("|")[1];
					const pref = `Usage: \n${prefix}quotemaker texto|marca\n\nEx :\n${prefix}quotemaker Vc é gay|Chakal`
					if (args.length < 1) return reply(pref)
					reply(ind.wait())
					anu = await fetchJson(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=random`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {caption: 'aí poha', quoted: mek})
					await limitAdd(sender)
					break
				//fadli 
				case 'pinterest':
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
              	    if (args.length < 1) return reply('cadê o texto, caralho?')
                    teks = `${body.slice(8)}`
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Este é o número do meu dono, não incomode ele ou vai levar block',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
				bo = args[0]
...
• Limite: ${limitawal-updated.limit}`
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'toimg':
				if (!isRegistered) return reply(ind.noregis())
				if (!isQuotedSticker) return reply('Responda/marque uma figurinha, burro!')
					reply(ind.wait())
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use o código do país e o número tudo junto')
					try {
				...
				case 'mining':
                      ...
                      reply(`Alá meu dono, toma aqui ${one}Xp zeu guei`)
                      }else{
				...
				reply("SUCESSO ao entrar em : *" + joen.gid + "*" )
				break
				case 'readmore':
				case 'more':
                ...
                client.sendMessage(from, 'Sucesso, alterar a descrição do grupo', text, {quoted: mek})
					break
           case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem você deseja rebaixar, burro!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*Perdeu adm otário*🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`esse crocodilo aqui-> @${mentioned[0].split('@')[0]} perdeu ADM kkkkk🐊`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem você deseja promover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `PARABÉNS VC É O NOSSO NOVO ADM (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`PARABÉNS🥳 @${mentioned[0].split('@')[0]} *Você é o nosso novo ADM👮🏻‍♂️* NÃO FAÇA NADA DE ERRADO PARA NÃO PERDER ESSE CARGO SUPREMO🐉`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem você quer banir, burro!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`SACA SÓ O PAI REMOVENDO ESSE CROCODILO AQUI🐊 @${mentioned[0].split('@')[0]} kkkkkkkk`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCESSO ❭ ativar boas-vindas/sair neste grupo ')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭  desativar boas-vindas/sair neste grupo ')
					} else {
						reply(ind.satukos())
					}
					break 
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCESSL ❭ ATIVAR NSFW NESTE GRUPO')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUCESSO ❭ DESLIGANDO O NSFW NESTE GRUPO')
					} else {
						reply(ind.satukos())
					}
				break
				...
				case 'clone':
					...
					if (args.length < 1) return reply(' *MARQUE QUEM VOCÊ QUER CLONAR, POHA!!!* ')
			  	...
			  	case 'event':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*❬ SUCESSO ❭ ATIVAR EVENTOS NESTE GRUPO*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ SUCESSO ❭ DESATIVANDO EVENTO NESTE GRUPO*')
					} else {
						reply(ind.satukos())
					}
					break 
				case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*❬ SUCESSO ❭ ANTILINK ATIVADO, VOU COMER O CÚ DE QUEM MANDAR LINK NESSA POHA!!!*')
					} else if (Number(args[0]) === 0) {
				...
				case 'setppbot':
					...
					reply('Obrigado pelo novo perfil, te mamo❤')
					break 
				case 'setpp': 
                        if (!isGroup) return reply(ind.groupo())
                       if (!isGroupAdmins) return reply(ind.admin())
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
                    prefix = args[0]
                    reply(`Prefixo trocado para ${prefix} com sucesso!`)
					break 
				case 'setmemlimit':
					if (args.length < 1) return
					if (!isOwner) return reply(ind.ownerb())
                    ...
                    memberlimit = args[0]
                    reply(`Alterar limite de membros para ${memberlimit} com sucesso!`)
				break 
				case 'bcgc':
				     if (!isOwner) return reply(ind.ownerb())
					if (args.length < 1) return reply('.......')
				...
				case 'tomp3':
				client.updatePresence(from, Presence.composing)
				if (!isQuotedVideo) return reply('isso é um vídeo mano?:V')
				reply(ind.wait())
				encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
				media = await client.downloadAndSaveMediaMessage(encmedia)
				ran = getRandom('.mp4')
